...
export const ChatProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // 응답이 있는지 여부 계산
  const hasResponse = messages.length > 0 || isAnimating || uiState === ChatUIState.Active;
  ...
  }, [currentAiModel]); // currentAiModel이 변경될 때마다 이벤트 리스너 업데이트

  // AI 모델 전환 함수
  const switchAiModel = async (model: string) => {
    ...
    try {
      ...
      // AI 전환 API 호출
      const response = await switchAIAndContinue(apiMessages, currentAiModel);
